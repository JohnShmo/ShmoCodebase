...
local_fn void set_rehash(set_t *s, size_t new_count) {
    assert(s);
    ...
    for (size_t i = 0; i < old_count; ++i) {
        ...
        while (bucket) {
            ...
            size_t hash = s->hash_func(bucket->elm) % new_count;
...
local_fn set_bucket_t *set_lookup(const set_t *s, const void *elm) {
    ...
    size_t hash = s->hash_func(elm) % s->slot_count;
...
void set_shrink(set_t *s) {
    ...
    set_rehash(s, (size_t)((f64)s->size * 1.25));
